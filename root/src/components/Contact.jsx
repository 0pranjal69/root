...
function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
const res = await axios.post("https://root-nd5n.onrender.com/send", data);
          ...
          <form className="flex flex-col gap-y-2 p-4" onSubmit={handleSubmit}>
            ...
            <div className="flex justify-center items-center gap-x-2 ">
              ...
              <div className="sm:w-[25vw] w-full">
                <button
  type="submit"
  className="amatic w-full relative px-5 py-1 text-2xl rounded-[10px] border-2 border-[#BBCE96] hover:border-0 cursor-pointer text-[#555451] overflow-hidden group z-10 text-center active:scale-95 transition-transform duration-150 ease-in-out"
>
  {/* Background hover animation */}
  <span className="absolute w-full h-full bg-[#BBCE96] top-full left-0 group-hover:top-0 transition-all duration-300 ease-in-out z-0"></span>

  {/* Button text above the background */}
